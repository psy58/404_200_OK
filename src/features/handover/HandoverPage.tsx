  ...
  return (
    <div className="stack">
      <div className="page-head">
        <div>
          <span className="eyebrow">{academicYear}학년도</span>
          <h1 className="t-display" style={{ marginTop: 9 }}>인수인계서 초안</h1>
          <p className="sub">올해 업무 기록으로 만든 인수인계 초안입니다.</p>
        </div>
        <div style={{ display: "flex", gap: 9 }}>
          <button className="btn btn-quiet" onClick={() => open("review")}>메모 검토 모드</button>
          <button className="btn btn-primary" disabled title="내보내기 기능은 준비 중입니다">내보내기</button>
        </div>
      </div>
      <div className="notice">
        <InfoIcon />
        <span><strong>아직 초안입니다.</strong> 승인·검증된 기록만 미리보기에 포함되며 내보내기는 실제 P1 계약 전까지 비활성입니다.</span>
      </div>
        ...
        {(preview) => (
          <div className="ho-doc">
            ...
            {preview.notes.map((note) => {
              ...
              return <div className="mini-note" style={{ marginTop: 12 }} key={note.id}><span style={{ display: "flex", gap: 7 }}><span className={`chip ${label.tone}`}>{label.label}</span><span className="chip">{note.taskTitle}</span></span><p>{note.body}</p><span className="mf">{note.authorDisplay} · {note.academicYear}학년도</span></div>;
            })}
            <h3>4. 주요 근거 문서</h3>
