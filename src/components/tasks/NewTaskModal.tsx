...
export function NewTaskModal({ onClose }: { onClose: () => void }) {
  ...
  const mutation = useMutation({
    mutationFn: () => {
      ...
      return createTask(context, { title: title.trim(), startDate, dueDate, memo: memo.trim() });
    },
    onSuccess: (task) => {
      if (context) {
    ...
    onError: (error) => toast(getSafeErrorMessage(error), "error"),
  });
      ...
      footer={
        <>
          ...
          <button
            ...
            disabled={!context || !title.trim() || mutation.isPending}
